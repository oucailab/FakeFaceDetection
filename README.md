...
本项目适用于 Deepfake 检测、AI 换脸识别、视频内容审核、身份真实性辅助判断等场景。检测结果应作为辅助参考，不能作为唯一判定依据。

本仓库提供了两个数据集 `FaceForensics++` 、 `Celeb-DF-v2` 的下载方式、一个可直接用于测试的训练权重文件和10个用于视频检测的样例，可在release页下载。
...
项目提供数据集级别的评估入口。请先将对应数据集放到 `data/` 目录，并保证目录结构与代码中的数据读取逻辑一致。

以 `Celeb-DF-v2` 测试集为例，目录结构如下：
...
```bash
python src/inference/inference_dataset.py -w output/weights/FFc23.tar -d CDF
```
...
| ---- | ---------------------------------------------------- |
| `-w` | 模型权重文件路径                                     |
| `-d` | 输入数据集类型，支持 CDF, FF, DFD, DFDC, DFDCP, FFIW |
...
训练前需要准备训练数据，本项目默认用 `FaceForensics++` 数据集进行训练。
...
预处理完成后，同一目录下会生成训练代码需要读取的 `frames/`、`landmarks/` 和 `retina/`：
...
当前训练代码会从 `data/FaceForensics++/original_sequences/youtube/c23/frames/` 读取训练帧，因此预处理命令需要显式指定 `-c c23`。如果下载数据集里的视频是 `raw` 类型， 目录生成预处理结果，当前训练入口不会读取到这些文件。
